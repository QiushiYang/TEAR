...
class CTAugment:
    ...
    def __call__(self, img, probe=True):
        ...
        return self.apply(Image.fromarray(img), policy)
...
if __name__ == '__main__':
    ...
    import os
    try:
        im = Image.open(os.path.dirname(os.path.dirname(__file__)) + '/data/test.png')
    except:
        raise IOError('Expect a test image under path ../data/test.png')
    cta = CTAugment()
    transformed = cta(im)
    plt.figure()
    plt.subplot(121)
